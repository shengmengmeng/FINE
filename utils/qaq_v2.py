...
class QaQ:
    ...
    @torch.no_grad()
    def masking(self, logits_corr, logits_s,*args, **kwargs):
        if not self.prob_max_mu_t.is_cuda:
            self.prob_max_mu_t = self.prob_max_mu_t.to(logits_corr.device)
        if not self.prob_max_var_t.is_cuda:
            self.prob_max_var_t = self.prob_max_var_t.to(logits_corr.device)

        probs = logits_corr.detach()
        _, label_corr = probs.max(dim=-1)
        self.update(probs)

        max_probs = logits_s[torch.arange(len(label_corr)), label_corr.long()]
        max_idx = label_corr
        # compute weight
        if not self.per_class:
