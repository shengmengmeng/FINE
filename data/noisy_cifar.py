...
class NoisyCIFAR_100N(torchvision.datasets.CIFAR100):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
                 noise_type='clean', closeset_ratio=0.0, openset_ratio=0.2, random_state=0, verbose=True):
        super().__init__(root, train=train, transform=transform, target_transform=target_transform,
                         download=download)
        ...
        if self.train and (noise_type != 'clean'):
            ...
            # noisy_labels, self.actual_noise_rate = noisify_dataset(self.nb_classes, train_labels, noise_type, closeset_ratio,
            #                                                        openset_ratio, random_state, verbose)
            self.noisy_labels = self.load_label().tolist()
    ...
    def load_label(self):
        ...
        if isinstance(noise_label, dict):
            if "clean_label" in noise_label.keys():
                ...
                self.train_labels = clean_label
                print(f'The overall noise rate is {1-np.mean(clean_label.numpy() == noise_label["noisy_label"])}')
            return noise_label['noisy_label'].reshape(-1)
        else:
